const HEADER_HTML = `<!-- TOP ANNOUNCEMENT BAR -->
...
function loadHeaderComponent() {
  ...
  // Render header HTML string immediately (supports both file:// and http:// protocols)
...
function initHeaderInteractivity() {
  ...
  triggers.forEach(trigger => {
    trigger.addEventListener('click', (e) => {
      ...
      // If it wasn't open before, open this specific menu
      if (!isOpen && menu) {
  ...
  // Stop click inside mega menu panel from closing it
  menus.forEach(menu => {
    menu.addEventListener('click', e => e.stopPropagation());
  });

  // Global click outside listener to close mega menus and account dropdowns
  ...
  accountTriggers.forEach(trigger => {
    trigger.addEventListener('click', (e) => {
      ...
      // Close mega menus and other account panels
      menus.forEach(m => m.classList.remove('open'));
      triggers.forEach(t => t.classList.remove('open'));
      document.querySelectorAll('.account-dropdown-panel').forEach(p => p.classList.add('hidden'));
      document.querySelectorAll('.chevron-icon').forEach(c => c.classList.remove('rotate-180'));
